...
# configure the log to write in mistakes and collection records
...
# get all symbols of spot market
market=binance.load_markets()
spot_symbols=[s for s in market.keys() if '/' in s and not ('UP/' in s or 'DOWN/' in s or 'PERP' in s or '-' in s)]

# sort symbols depends on volume and get popular symbols(up to 100)
# get every symbol's volume during 24 hours and sort then get 100 popular symbols
...
def detect_large_trades(trades, symbol):
    ...
    # calculate the volume of sell and buy
...
def get_and_save_spot_seconds_data(symbol):
    ...
    try:
        ...
        if len(df)>=10:
            ...
            # get bid price and ask price, bid volume and ask volume
